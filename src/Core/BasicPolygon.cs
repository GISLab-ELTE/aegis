...
namespace AEGIS
{
    ...
    public class BasicPolygon : IBasicPolygon
    {
        ...
        /// </summary>
        private const String LineStringFormat = "({0})";

        /// <summary>
        /// The divider for line strings. This field is constant.
        /// </summary>
        private const String LineStringDivider = ",";
        ...
        public BasicPolygon(IReadOnlyList<Coordinate> shell)
            : this(shell, null)
        {
        }
        ...
        public BasicPolygon(IReadOnlyList<Coordinate> shell, IEnumerable<IReadOnlyList<Coordinate>> holes)
        {
            ...
            if (shell.Count == 0)
            {
                this.Shell = new BasicLineString();
            }
            else
            {
                if (shell[0].Equals(shell[shell.Count - 1]))
                    this.Shell = new BasicLineString(shell);
                else
                    this.Shell = new BasicLineString(shell.Concat(new Coordinate[] { shell[0] }));
            }
            ...
            if (holes != null)
            {
                foreach (IReadOnlyList<Coordinate> hole in holes)
                {
                    ...
                    if (hole[0].Equals(hole[hole.Count - 1]))
                        this.holes.Add(new BasicLineString(hole));
                    else
                        this.holes.Add(new BasicLineString(hole.Concat(new Coordinate[] { hole[0] })));
                }
            }
        }
        ...
        public String ToString(IFormatProvider provider)
        {
            ...
            builder.Append(String.Format(provider, LineStringFormat, partBuilder.ToString()));

            for (Int32 holeIndex = 0; holeIndex < this.holes.Count; holeIndex++)
            {
                builder.Append(LineStringDivider);
